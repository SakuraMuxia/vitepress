...
3. 在搜索页面配置router-link：src->pages->Search->index.vue
...
2. 在store中存储商品的数据：src->store->product.js
...
3. 在页面中调用：src->Pages->Details->index.vue
...
3. 在页面中引入：src->pages->Detail->Zoom->index.vue
...
2. 实现缩略图切换：src->store->product.js
...
2. 在缩略图的组件中修改：src->pages->Detail->Thumbnail->index.vue
