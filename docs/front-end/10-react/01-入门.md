...
```shell
两种方式：
一种是使用 npm install 安装bootstrap
一种是把bootstrap样式放在public目录中，然后在public目录的index.html中的link标签中引入bootstrap

```

详见：React->50-组件练习章节->Css样式处理
...
2. 三元表达式: {条件?<div>111</div>:</div>222</div>}</div>
3. 逻辑运算表达式 {条件 && <div>222</div>}</div>
4. 函数调用:return 返回一个值 {fn()}
...
number：正常输出
string：正常输出
boolean:不输出内容 block={true}可简写为block
null,undefined:不输出内容
对象:直接使用对象输出内容会报错(设置属性时，插值表达式内可以使用对象形式)
数组: 遍历每一个元素输出
...
import ReactDOM from 'react-dom';// ReactDOM 从 react-dom导入
import App from './App'

// 不创建根节点，直接使用ReactDOM对象渲染
